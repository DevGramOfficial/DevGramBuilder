...
def edit_ignore(args: argparse.Namespace, delete: bool) -> None:
    root = _project_root()
    state = _state(root)
    key = {"all": "ignoreAll", "no_assets": "optionalAssets"}[args.target]
...
def upload(args: argparse.Namespace) -> None:
    ...
    if args.archive:
        ...
        build_args = argparse.Namespace(
            no_assets=False, no_folder=True, verbose=args.verbose,
            ast=True, no_info=False, static_version=None,
...
def make_parser() -> argparse.ArgumentParser:
    ...
    build.add_argument("-a", "--ast", action="store_true", help="проверить синтаксис Python")
