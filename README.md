...
Пакет можно зашифровать паролем с помощью AES. DevGram запросит пароль при
установке:
...
Поддерживаются `aes-128`, `aes-192` и `aes-256`. Пароль не записывается в
манифест или журнал сборки. Без правильного пароля прочитать пакет невозможно.
...
Зашифрованный пакет можно отправить через Dev Server, передав пароль отдельно:

```bash
dgb upload builds/plugin.dgplugin --token "ТОКЕН" --package-password "ПАРОЛЬ"
```

Вместо аргумента поддерживается переменная `DEVGRAM_PLUGIN_PASSWORD`, чтобы не
оставлять пароль в истории команд.
