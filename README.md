...
Builder охватывает весь процесс разработки: создание проекта, проверку
открытых Python-исходников, сборку, watch-режим, ignore-правила, статистику
и загрузку через Dev Server. Результат собирается в нативный формат DevGram.
...
Пакет всегда содержит открытые файлы `.py`: байткод `.pyc` не собирается и
не принимается DevGram.
